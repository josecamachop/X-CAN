function fig_h = plot_vec2(vec,elabel,classes,xylabel,lcont,opt,vlabel,mult,maxv)

% Bar or line plot.
%
% plot_vec2(vec) % minimum call
% plot_vecp2(vec,elabel,classes,xylabel,lcont,opt,vlabel,mult,maxv) % complete call
%
%
% INPUTS:
%
...
if ~isempty(classes)
    ...
    for i=1:length(unique_classes)
        ...
        if opt == '0',
            plot(vec(ind,:)', 'Color', color_list(i,:), 'DisplayName', num2str(unique_classes(i)));
        else 
            bar(vind, vec(ind,:), length(find(ind))/N, 'FaceColor', color_list(i,:), 'EdgeColor', 'none', 'DisplayName', num2str(unique_classes(i)));
        end
    end 
else
    color_list = hsv(M);
